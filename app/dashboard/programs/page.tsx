...
export default async function ProgramsPage() {
  ...
  // Build search data for ProgramSearchBar
  const searchData = Array.from(grouped.entries()).map(([key, progs]) => ({
    key,
    name: progs[0].name,
    department: progs[0].department,
    degrees: [...new Set(progs.map(p => p.degree))],
    universities: progs.map(p => ({
    ...
    <div>
      ...
      <div style={{ marginTop: "24px" }}>
        {Array.from(grouped.entries()).length === 0 ? (
          ...
          <ProgramSearchBar programs={searchData}>
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
              {Array.from(grouped.entries()).map(([key, progs]) => {
                const first = progs[0];
                return (
                  <div key={key} data-program-key={key} style={{ backgroundColor: "white", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", overflow: "hidden" }}>
                    {/* Program Header */}
