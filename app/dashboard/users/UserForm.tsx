...
export default function UserForm() {
  ...
  const [form, setForm] = useState({
    firstName: "", lastName: "", email: "", password: "", phone: "", role: "CONSULTANT",
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setForm({ firstName: "", lastName: "", email: "", password: "", phone: "", role: "CONSULTANT" });
        setOpen(false);
        router.refresh();
      } else {
  ...
  return (
    <div style={{ backgroundColor: "white", padding: "24px", borderRadius: "12px", boxShadow: "0 2px 8px rgba(0,0,0,0.04)", marginBottom: "16px" }}>
      <h3 style={{ color: "#001459", fontSize: "16px", fontWeight: "700", marginBottom: "16px" }}>Nouvel utilisateur</h3>
      <form onSubmit={handleSubmit}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "16px" }}>
          ...
          <div>
            <label style={{ display: "block", marginBottom: "4px", fontSize: "12px", color: "#666", fontWeight: "600" }}>Role *</label>
            <select required style={{ ...inputStyle, cursor: "pointer" }} value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}>
              <option value="CONSULTANT">Consultant</option>
              <option value="SUB_AGENT">Sous-Agent</option>
